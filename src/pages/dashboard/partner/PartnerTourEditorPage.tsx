...
export default function PartnerTourEditorPage() {
    const { id } = useParams();
                ...
                try {
                    ...
                    if (response.success && response.data) {
                        ...
                        if (Array.isArray(t.destinations) && t.destinations.length > 0) {
                            ...
                            uiDestinations = [{ id: t.destination._id || t.destination, note: '', name: t.destination.name || '' }];
                        } else {
    ...
    useEffect(() => {
        const fetchDestinations = async () => {
            try {
                const response = await apiClient.get('/partner/destinations');
                if (response.success && response.data) {
                    setDestinationList(response.data);
                }
            } catch (err) {
                console.error('Could not fetch destinations', err);
            }
        };
        fetchDestinations();
    }, []);
    ...
    async function save(mode: 'draft' | 'publish') {
        try {
            ...
            let response;
            if (isNew) {
                response = await apiClient.post('/partner/tours', payload);
            } else {
                response = await apiClient.put(`/partner/tours/${id}`, payload);
            }
    ...
    return (
        <div className="p-6 space-y-4">
            <div className="sticky top-0 z-10 bg-background/80 backdrop-blur border-b border-border py-3 flex items-center gap-2">
                <Button variant="ghost" onClick={() => navigate('/dashboard/tours')}>{'<'} Quay lại</Button>
            ...
            <Tabs value={tab} onValueChange={setTab}>
                <TabsList>
                    ...
                    <TabsTrigger value="reviews" disabled={isNew}>Đánh giá</TabsTrigger>
                </TabsList>
                                ...
                                <div className="space-y-2">
                                    {(doc.destinations || []).map((d: any, idx: number) => (
                                        <div key={idx} className="flex items-start gap-2">
                                            ...
                                            <input className="w-48 rounded border border-border px-2 py-2 bg-background" placeholder="Ghi chú" value={d.note || ''} onChange={(e) => updateDestinationAt(idx, { note: e.target.value })} />
                ...
                <TabsContent value="reviews">
                    <Card className="p-4 mt-4">
                        <div className="text-sm text-muted-foreground p-4 text-center">
                            Tính năng xem đánh giá chi tiết đang được phát triển
                        </div>
                    </Card>
                </TabsContent>
            </Tabs>
