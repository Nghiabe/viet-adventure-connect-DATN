...
export async function seedHaLong() {
  const haLongBayData = {
    name: 'Vịnh Hạ Long',
    slug: 'ha-long-bay',
    description:
  ...
  const createdDestination = await Destination.create(haLongBayData);
  const toursWithDestinationId = toursForHaLong.map((tour) => ({
    ...tour,
    destination: createdDestination._id,
  }));
  await Tour.insertMany(toursWithDestinationId);

  return { destination: createdDestination, tours_count: toursWithDestinationId.length };
}
